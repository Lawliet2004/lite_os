...
| 8.9: User Management | `/etc/passwd`, `/etc/shadow`, login, su, passwd, useradd/userdel with SHA-256 hashing and credential enforcement. | ✅ Completed |
| 8.10: Self-hosted Build | Build a simple C program inside LiteNix using an in-kernel-hosted toolchain. | 📋 Queued |
| 9.0: VFS Permission Enforcement + Setuid | Unix mode-bit checks in open/unlink/mkdir/rename/..., S_ISUID bit honored in execve, /bin/login + /bin/passwd + /bin/su installed setuid root, /etc/shadow mode 0600 preserved across updates. | ✅ Completed |
| 9.1: DHCP client + hostname | `/sbin/dhcpcd` (DISCOVER/OFFER/REQUEST/ACK), `/bin/hostname` backed by `/proc/sys/kernel/hostname`, rcS.sh prefers DHCP then falls back to static, libc-lite `dhcp_parse_reply` shared with init Test 34. | ✅ Completed |
| 9.2: System logging | `/sbin/klogd` tails `/dev/kmsg` into `/var/log/kern.log`; `/bin/logger` writes userspace messages to the kernel ring buffer; rcS.sh starts klogd as a daemon and records a "boot complete" marker; libc-lite adds `strstr`; init Test 35 covers the round trip. | ✅ Completed |
| 9.3: Real service supervision | Replace the shell-based `service` with a C-based `/sbin/svc` and a separate `/sbin/supervisor` respawn daemon. Services live under `/etc/services.available/<name>.conf` with `KEY="VALUE"` syntax and `AFTER="..."` dependency ordering. `rcS.sh` runs `svc start-enabled` and `supervisor --daemon` at boot. init Test 36 covers start/stop/status/respawn round trip. | ✅ Completed |
| 9.4: Service log inspection | Add `svc <name> log` to read the per-service log file (`LOG_FILE` from the .conf, falling back to `/var/log/services/<name>.log`). 64 KiB cap per dump; distinct return codes for success/missing-log/read-error. Documented in `docs/supervisor.md`. | ✅ Completed |
| 9.5: Service describe | Add `svc <name> describe` to dump the parsed .conf fields (NAME/DESCRIPTION/EXEC/ARGV/AFTER/RESPAWN/MAX_RESTARTS/LOG_FILE) plus runtime state (enabled flag, status, pid file path, log file path) in a greppable KEY=VALUE format. Lets operators inspect a service definition without re-reading the .conf. | ✅ Completed |
| 9.6: Service dependency tree | Add `svc tree` to walk `/etc/services.available/<name>.conf` and print the `AFTER="..."` graph as a tree, with each node annotated by its live `state` (`running`/`stopped`/`crashed`) and `enabled` flag. Roots are services that nothing depends on; cycles and unreachable nodes are flagged at the bottom. Lets operators see the actual boot order without reading every .conf. | ✅ Completed |
| 9.7: Service check | Add `svc check [name]` to validate on-disk service state before boot: parses every .conf, walks `EXEC=` to confirm the binary is reachable, walks `AFTER=` to confirm every dependency name resolves to a defined service, and cross-checks `/etc/services.enabled` for orphan enables (service listed but `.conf` missing). Returns 0 if all OK, 1 if any issues. Designed to be safe to call from recovery mode. | ✅ Completed |
