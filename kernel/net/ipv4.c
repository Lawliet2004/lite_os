...
{
    uint16_t ip_len = sizeof(struct ipv4_hdr) + len;
    uint8_t *ip_buffer = kmalloc(ip_len);
    if (!ip_buffer) return;

    struct ipv4_hdr *ip = (struct ipv4_hdr *)ip_buffer;
    ip->ver_ihl = 0x45; // Version 4, IHL 5 (20 bytes)
    ip->tos = 0;
    ip->len = (uint16_t)((ip_len >> 8) | (ip_len << 8));
    ...
    if (dest_ip[0] == 127 || memcmp(dest_ip, my_ip, 4) == 0) {
        net_ipv4_receive(ip_buffer, ip_len);
        kfree(ip_buffer);
        return;
    }
    ...
    if (retries >= 0) {
        net_eth_send(dest_mac, ETH_TYPE_IPV4, ip_buffer, ip_len);
    }
...
void net_ipv4_receive(const void *data, uint16_t len)
{
    ...
    // Verify target IP is my_ip, loopback, or broadcast (like 255.255.255.255 or subnet broadcast)
    if (memcmp(ip->dest, my_ip, 4) != 0 && ip->dest[0] != 127 && ip->dest[3] != 255) {
        return;
    }
